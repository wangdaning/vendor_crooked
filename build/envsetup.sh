...
cat <<EOF
Additional ScorpionROM functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- scorpiongerrit:   A Git wrapper that fetches/pushes patch from/to ScorpionROM Gerrit Review.
- scorpionrebase:   Rebase a Gerrit change and push it again.
- scorpionremote:   Add git remote for ScorpionROM Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cafremote:       Add git remote for matching CodeAurora repository.
- githubremote:    Add git remote for ScorpionROM Github.
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/scorpion-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.scorpion.device | grep -q "$SCORPION_BUILD"); then
...
function scorpionremote()
{
    ...
    git remote rm scorpion 2> /dev/null
    ...
    then
        ...
        local PFX="ScorpionROM/"
    else
        local PROJECT=$REMOTE
    fi

    local SCORPION_USER=$(git config --get review.gerrit.scorpionrom.com.username)
    if [ -z "$SCORPION_USER" ]
    then
        git remote add scorpion ssh://gerrit.scorpionrom.com:29418/$PFX$PROJECT
    else
        git remote add scorpion ssh://$SCORPION_USER@gerrit.scorpionrom.com:29418/$PFX$PROJECT
    fi
    echo "Remote 'scorpion' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/ScorpionROM/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        scorpionremote
        git push scorpion HEAD:refs/heads/'$1'
    fi
    '
}

function scorpiongerrit() {
    ...
    local user=`git config --get review.gerrit.scorpionrom.com.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "scorpiongerrit" ]; then
...
function scorpionrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "ScorpionROM Gerrit Rebase Usage: "
        echo "      scorpionrebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://gerrit.scorpionrom.com/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
