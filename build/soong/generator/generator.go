...
func init() {
	android.RegisterModuleType("crooked_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = crookedExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = crookedExpandVariables(ctx, dep_file)
	...
	cmd := crookedExpandVariables(ctx, String(g.properties.Cmd))
